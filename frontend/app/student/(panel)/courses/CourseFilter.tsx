...
export default function CourseFilter({ courses, categories }: Props) {
  ...
  const featuredCourse = filtered[0];
  const otherCourses = filtered.slice(1);
  ...
  return (
    <>
      ...
      {featuredCourse && (
        <div className="px-4 md:px-8 max-w-7xl mx-auto mb-12 md:mb-24 mt-6">
          <Link
          ...
          >
            <div className="absolute inset-0 md:relative md:w-1/2 h-full">
              <Image src={featuredCourse.thumbnail} alt={featuredCourse.title} fill className="object-cover opacity-50 md:opacity-100 group-hover:scale-105 transition-transform duration-1000" sizes="(max-width: 768px) 100vw, 50vw" />
            ...
            <div className="relative z-10 p-6 sm:p-8 md:p-16 flex flex-col justify-end md:justify-center md:w-1/2 h-full min-h-[360px] md:min-h-[400px]">
              <div className="mb-3 md:mb-4 mt-auto md:mt-0">
                <span className="text-[9px] md:text-[10px] font-bold tracking-widest uppercase border border-white/30 px-2.5 py-1 md:px-3 md:py-1.5 rounded-full backdrop-blur-md">Featured</span>
              </div>
              <h3 className="text-2xl sm:text-3xl md:text-5xl font-serif font-bold leading-tight mb-3 md:mb-6 text-balance">{featuredCourse.title}</h3>
              <p className="text-white/80 text-sm md:text-lg font-light mb-6 md:mb-8 max-w-md line-clamp-2 md:line-clamp-3">{featuredCourse.description}</p>
        ...
        <div className="px-4 md:px-8 max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 lg:gap-x-12 gap-y-10 md:gap-y-16">
            {otherCourses.map((course) => (
              <Link key={course.id} href={`/student/lesson/${course.id}`} className="group cursor-pointer flex flex-col">
                <div className="aspect-[4/3] rounded-2xl md:rounded-3xl overflow-hidden mb-4 md:mb-6 relative">
                  <Image src={course.thumbnail} alt={course.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" />
                  <div className="absolute top-3 left-3 md:top-4 md:left-4">
                    <span className="px-2.5 py-1 md:px-3 md:py-1.5 bg-white/90 backdrop-blur-md text-on-surface text-[9px] md:text-[10px] font-bold rounded-full uppercase tracking-widest">{course.category}</span>
                  </div>
                </div>
                <div className="flex-1 flex flex-col">
                  <h3 className="font-serif font-bold text-xl md:text-2xl text-on-surface leading-tight mb-2 md:mb-3 group-hover:text-primary transition-colors text-balance">{course.title}</h3>
                  <p className="text-secondary text-sm md:text-base font-light line-clamp-2 mb-4 md:mb-6 flex-1">{course.description}</p>
      ...
      {filtered.length === 0 && (
        <div className="px-4 md:px-8 max-w-7xl mx-auto py-16 text-center text-secondary">
          このカテゴリのコースはありません
        </div>
      )}
    </>
